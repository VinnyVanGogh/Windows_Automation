...
$tempFolderPath = Join-Path -Path $desktopPath -ChildPath "ScriptedByVinny"
...
$mobileVPNShortcutPath = Get-ChildItem -Path $desktopPath | Where-Object { $_.Name -like "*Mobile VPN*" }
...
if ($confirmation -eq "Y" -or $confirmation -eq "Yes") {
    ...
    $uninstallerPath = "C:\Program Files (x86)\WatchGuard\WatchGuard Mobile VPN with SSL\unins000.exe"

    if (Test-Path $uninstallerPath) {
        $uninstallArgs = "/S"
        $uninstallResult = Start-Process -FilePath $uninstallerPath -ArgumentList $uninstallArgs -Wait -NoNewWindow
        ...
        Write-Host "Uninstaller not found at: $uninstallerPath"
    }
...
if ($uninstallTeamsOption -eq "Y") {
    ...
    Start-Process -FilePath $teamsUpdatePath -ArgumentList "--uninstall" -NoNewWindow -Wait
}
else {
...
Start-Process -Wait -FilePath $ODTExePath -ArgumentList "/quiet", "/extract:$ODTTempFolderPath"
...
if ($installSaRAOption -ne "Y") {

$arglist = "/quiet"

Invoke-WebRequest -Uri $SaRADownloadURL -OutFile $SaRASetupPath

Start-Process -FilePath $SaRASetupPath -ArgumentList $arglist -Wait
