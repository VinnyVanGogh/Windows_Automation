# Script to install Microsoft 365 Business with Teams, and watchguard and create shortcuts, plus a welcome doc
...
if ($installWatchGuardOption -in @("Y", "y", "Yes", "yes", "YES")) {
    ...
    Write-Host "WatchGuard Mobile VPN installation and configuration completed." -ForegroundColor DarkGreen
...
if ($installTeamsOption -eq "Y") {
    Write-Host "Starting installation of Microsoft Teams..." -ForegroundColor DarkGreen
...
Write-Host "Changing Power settings..." -ForegroundColor DarkGreen
...
if (!(Test-Path "$desktopPath\*Mobile VPN*.lnk")) {
    ...
    Write-Host "Shortcut for $domainName WatchGuard Mobile VPN has been created..." -ForegroundColor DarkGreen
    $vpnShortcutcreated = $true
}
else {
    Write-Host "Unable to create shortcut $loggedInUser's Microsoft Teams for $domainName, you can find the application at, $watchguardShortcut" -ForegroundColor DarkRed
}
...
if (!(Test-Path "$desktopPath\Microsoft Teams (work or school).lnk")) {
  ...
  Write-Host "Shortcut created for $loggedInUser's Microsoft Teams for $domainName" -ForegroundColor DarkGreen
  $teamsShortcutcreated = $true
}
else {
  Write-Host "Unable to create shortcut $loggedInUser's Microsoft Teams for $domainName, you can find the application at, $teamsShortcut" -ForegroundColor DarkRed
}
...
foreach ($app in $appsToShortcut) {
    ...
    if (-not (Test-Path $shortcutPath)) {
        ...
        $shortcut.Save()
        Write-Host "Shortcut created for $app." -ForegroundColor DarkGreen
    } else {
        Write-Host "Shortcut already exists for $app." -ForegroundColor DarkYellow
    }
    $shortcutCreated = Test-Path $shortcutPath
    Write-Host "$app : $(if ($shortcutCreated) { 'Shortcut created successfully...' } else { "Failed to create shortcut for $app you can create one from $targetPath" })" -ForegroundColor DarkYellow
}


Write-Host "WatchGuard Mobile VPN: $(if ($vpnShortcutcreated) { 'Shortcut created successfully...' } else { "Failed to created shortcut, you can create one from $watchguardShortcut" })" -ForegroundColor DarkYellow
Write-Host "Microsoft Teams: $(if ($teamsShortcutcreated) { 'Shortcut created successfully...' } else { "Failed to created shortcut, you can create one from $teamsShortcut" })" -ForegroundColor DarkYellow
...
try {
  ...
  Write-Host "Microsoft Store has been reset successfully." -ForegroundColor DarkGreen
} catch {
  Write-Host "Failed to reset the Microsoft Store: $_" -ForegroundColor Red
}

Write-Host "Applications installed, creating Welcome Document for user." -ForegroundColor DarkGreen
...
$docPath = "$desktopPath\Welcome Info - From Accent.docx"
...
if ($renamePerformed -and (Read-Host -Prompt "Do you want to reboot the computer? (Y/N)") -eq "Y") {
  Write-Host "Script completed successfully, rebooting in 3 seconds. Enjoy :)" -ForegroundColor DarkGreen
