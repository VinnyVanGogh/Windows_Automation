<#
This script retrieves Office 365 licensing information for users in Azure Active Directory, 
saves it to a CSV file in the specified folder,
formats it in Markdown syntax, 
and sets it to the clipboard for further use.
#>
Import-Module AzureAD
Connect-AzureAD 

$vc3_folder_path = "C:\VC3"
$csv_file_prefix = "O365_Licensing_Info"
$csv_filename = Join-Path -Path $vc3_folder_path -ChildPath ($csv_file_prefix + ".csv")
$vc3_folder_params = @{
    ItemType = 'Directory'
    Path = $vc3_folder_path
}

if (!(Test-Path -PathType Container $vc3_folder_path)) {
    New-Item @vc3_folder_params
}
...
function Get-MicrosoftLicensingInfo {
    $license_info = New-Object 'System.Collections.ArrayList'
    $markdown_license_info = @{}
    $total_licenses = @{}
    foreach ($user in Get-AzureADUser -All $true) {
        ...
        try {
            ...
            Write-Host "Error retrieving licenses for user: $user_principal_name"
            $licenses = @()
        }

        $subscribed_skus = Get-AzureADSubscribedSku | Group-Object -Property SkuId -AsHashTable -AsString

        foreach ($license in $licenses) {
            $sku_id = $license.SkuId
            $license_name = $subscribed_skus["$sku_id"].SkuPartNumber

            $total_licenses[$license_name] = $total_licenses[$license_name] + 1
            
            
            $null = $license_info.Add([PSCustomObject]@{
                'UserPrincipalName' = $user_principal_name
                'SkuId' = $sku_id
                'LicenseName' = $license_name
                'TotalLicenses' = $total_licenses[$license_name]
            })

            if ($license_map.ContainsKey($license_name)) {
                $license_name = $license_map[$license_name]
            }

            if (-not $markdown_license_info.ContainsKey($license_name)) {
                $markdown_license_info[$license_name] = New-Object 'System.Collections.Generic.List[System.Object]'
            }

            $markdown_license_info[$license_name].Add($user_principal_name)
        }
    }
    $license_info | Export-Csv -Path $csv_filename -NoTypeInformation
    return $markdown_license_info
}
function Format-InMarkdown {
    param (
        [hashtable]$markdown_license_info
    )
    $markdown_output = "## Office 365 Licenses`r`n"
    foreach ($license in $markdown_license_info.GetEnumerator()) {
        $markdown_output += "### " + $license.Key + "`r`n"
        $markdown_output += "    - License count: **" + $license.Value.Count + "**`r`n"
        foreach ($user in $license.Value) {
            $markdown_output += "        - " + $user + "`r`n"
        }
    }
    Set-Clipboard -Value $markdown_output
}

Get-MicrosoftLicensingInfo
$markdown_license_info = Get-MicrosoftLicensingInfo
Format-InMarkdown -markdown_license_info $markdown_license_info
